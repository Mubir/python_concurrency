...
class ThumbnailMakerService(object):
    ...
    def download_image(self):
        while not self.dl_queue.empty():
            try:
                #if queue is empty for block=false will throw an exception 
    ...
    def make_thumbnails(self, img_url_list):
       ...
       # t1 = Thread(target=self.download_images,args=([img_url_list]))
        ...
        t2.start()
        # to finish all the download then sending the poision pill 
        # it blocks the main thread and finish hte dwonload 
        # we dont add poison pill in dwonalod image method because 
        # in that case one of the four may finish and add the poision pill
        # and poision pill will not be added at the last;
        self.dl_queue.join()
        self.img_queue.put(None)
        t2.join()
